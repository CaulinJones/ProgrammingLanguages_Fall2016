...
(define value-of-cps
  (lambda (expr env-cps k)
    (match expr
      ...
      [`(letcc ,body)(value-of-cps body (lambda(y k^)(if (zero? y) (apply-k k^ k) (apply-env env-cps (sub1 y) k^))) k)]
      ...
      [`(lambda ,body)(apply-k k(lambda (a k^)(value-of-cps body (lambda (y k^) (if (zero? y) (apply-k k^ a) (apply-env env-cps (sub1 y)k^)))k^)))]
